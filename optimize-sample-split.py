...
for sample_size in sample_size_space:  # 19 iterations
    for sample_min_count in sample_min_count_space:  # 30 iterations
        sample_df = multilabel_sample_dataframe(df[LABELS],  # no features as it's just a simulation
                                                y, size=sample_size, min_count=sample_min_count, seed=1)
        sample_labels = pd.get_dummies(sample_df[LABELS], prefix_sep='__')
        sample_ratio = (sample_labels.value_counts() / y_counts).mean()
        # The sample less frequent class upper limits the split min_count otherwise it fails
        sample_less_frequent_class = sample_labels.sum().min()
        for test_size in np.linspace(0.1, 0.9, 9):
            ...
            for test_min_count in test_min_count_space:
                # The test_size limits the test_min_count
                if sample_labels.shape[1] * test_min_count > np.floor(sample_labels.shape[0] * test_size):
                    continue
                ...
                msg = 'sample_size {:.2f} min_count {} less_freq {} sample_ratio {:.5f} test_size {:.1f} min_count {} '\
                      'train_less_freq {} test_lessfreq {} train_ratio {:.5f} test_ratio {:.5f}'
